...
import lchj.kt.user.utils.file.FileInfoUtils.getFileSize
...
object DataCleanManager {
    ...
    fun getTotalCacheSize(context: Context): String {
        ...
        if (Environment.getExternalStorageState() == Environment.MEDIA_MOUNTED) {
            try {
                externalCacheSize = getFileSize(context.externalCacheDir)
            } catch (e: Exception) {
                e.printStackTrace()
            }
        ...
        try {
            cacheSize = getFileSize(context.cacheDir)
        } catch (e: Exception) {
            e.printStackTrace()
        }

        try {
            filesSize = getFileSize(context.filesDir)
        } catch (e: Exception) {
            e.printStackTrace()
        }

        val s = FileInfoUtils.FormetFileSize(externalCacheSize + cacheSize + filesSize)
        Log.e("lchj", "s:" + s)
